...
process_constructor_function <- function(constructor, data_type_name, env) {
    ...
    constructor <- function() {
        ...
        for (i in seq_along(args)) {
            ...
            stopifnot(rlang::is_na(type) || type %in% class(arg))
        }
...
process_constructor_constant <- function(constructor, data_type_name, env) {
    ...
    constructor_object <- structure(NA, constructor_constant = constructor_name, 
...
#' two argumetns, \code{car} and \code{cdr}, and requires that \code{cdr} has type
