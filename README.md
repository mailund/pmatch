...
The `case_func` function works by matching its first element–which
...
When we use `case_func` to match against such patterns, we can bind
...
If you want to test more than one pattern at a time you can create a
data type for tuples, e.g.
